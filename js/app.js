...
// Local role assignments (in-app simulation for roles since Identity admin API
// requires server-side calls; this tracks role assignments in localStorage)
function getRoleAssignments() {
  try {
    return JSON.parse(localStorage.getItem("gltp_roles") || "{}");
  } catch (e) { return {}; }
}

function saveRoleAssignment(playerName, role) {
  const assignments = getRoleAssignments();
  assignments[playerName] = role;
  localStorage.setItem("gltp_roles", JSON.stringify(assignments));
}

function getPlayerRole(playerName) {
  const assignments = getRoleAssignments();
  return assignments[playerName] || "player";
}
...
function renderUserManagement() {
  ...
  // Combine all players from teams
  const allPlayers = [];
  teams.forEach(t => {
    t.players.forEach(p => {
      allPlayers.push({ ...p, teamId: t.id });
    });
  });

  tbody.innerHTML = allPlayers.map(p => {
    const role = getPlayerRole(p.name);
    return `<tr>
      <td><div class="player-cell"><span class="player-avatar-sm">${initials(p.name)}</span>${p.name}</div></td>
      <td>Team ${p.teamId}</td>
      <td>${p.handicap}</td>
      <td>
        <select class="role-select" data-player="${p.name.replace(/"/g, '&quot;')}" onchange="handleRoleChange(this)">
      ...
      <td><span class="role-badge role-${role}" style="font-size:.7rem;">${role === 'admin' ? 'Admin' : role === 'scorer' ? 'Scorer' : 'Player'}</span></td>
    </tr>`;
  }).join("");
}

function handleRoleChange(sel) {
  const playerName = sel.dataset.player;
  const newRole = sel.value;
  saveRoleAssignment(playerName, newRole);
  renderUserManagement();
}
// Make it globally accessible for inline onchange
window.handleRoleChange = handleRoleChange;

// ---- Admin Teams Grid ----
function renderAdminTeamsGrid() {
  const grid = document.getElementById("adminTeamsGrid");
  if (!grid) return;
  grid.innerHTML = teams.map(t => {
    const combined = t.players[0].handicap + t.players[1].handicap;
    const p1Role = getPlayerRole(t.players[0].name);
    const p2Role = getPlayerRole(t.players[1].name);
    return `<div class="team-card">
      <div class="team-number">Team ${t.id}</div>
      ${t.players.map((p, idx) => {
        const r = idx === 0 ? p1Role : p2Role;
        return `<div class="team-player">
          <span class="player-avatar-sm">${initials(p.name)}</span>
          <span class="team-player-name">${p.name}</span>
          <span class="team-player-hcp">HCP ${p.handicap}</span>
          <span class="role-badge role-${r}" style="font-size:.6rem;margin-left:auto;">${r === 'admin' ? 'A' : r === 'scorer' ? 'S' : 'P'}</span>
        </div>`;
      }).join("")}
      <div class="team-combined">
        <span>Combined HCP</span>
...
function renderHandicapTable() {
  ...
  tbody.innerHTML = teams.map(t => {
    ...
    const p2 = t.players[1];
    const hcp1_9 = get9HoleHcp(p1.handicap);
    const hcp2_9 = get9HoleHcp(p2.handicap);
    const combined = p1.handicap + p2.handicap;
    allHcps.push(p1.handicap, p2.handicap);
    combinedHcps.push(combined);
    return `<tr>
      <td><strong>Team ${t.id}</strong></td>
      <td>${p1.name}</td>
      <td>${p1.handicap}</td>
      <td>${hcp1_9}</td>
      <td>${p2.name}</td>
  ...
  // Update stat cards
  const avg = (allHcps.reduce((s, v) => s + v, 0) / allHcps.length).toFixed(1);
  const low = Math.min(...allHcps);
  const high = Math.max(...allHcps);
  const teamAvg = (combinedHcps.reduce((s, v) => s + v, 0) / combinedHcps.length).toFixed(1);

  const el = (id, val) => { const e = document.getElementById(id); if (e) e.textContent = val; };
  el("hcpAvgAll", avg);
  el("hcpLow", low);
  el("hcpHigh", high);
  el("hcpTeamAvg", teamAvg);
}

// ---- My Stats (Player View) ----
function renderMyStats() {
  const user = window.GLTP_USER || {};
  const userName = user.name || "";
  const contentDiv = document.getElementById("myStatsContent");
  const historyTbody = document.getElementById("myScoreHistory");
  const teamDiv = document.getElementById("myTeamInfo");
  if (!contentDiv) return;

  // Find the player in data
  const player = players.find(p => p.name.toLowerCase() === userName.toLowerCase());

  if (player) {
    contentDiv.innerHTML = `
      <div class="stats-row" style="padding:1rem;">
        <div class="stat-card">
          <div class="stat-info">
            <span class="stat-value">${player.handicap}</span>
            <span class="stat-label">My Handicap</span>
          </div>
        </div>
        <div class="stat-card">
          <div class="stat-info">
    ...
    contentDiv.innerHTML = `<p style="padding:1rem;color:var(--gray-600);">Player data not found for "${userName}". Contact your league admin.</p>`;
  }

  // My score history
  if (historyTbody) {
    const myScores = scoreHistory.filter(s => s.player.toLowerCase() === userName.toLowerCase());
    if (myScores.length > 0) {
      historyTbody.innerHTML = myScores.map(s => {
        const diff = s.score - s.par;
        const diffStr = diff === 0 ? "E" : (diff > 0 ? `+${diff}` : `${diff}`);
        const diffColor = diff < 0 ? "color:var(--green-600)" : diff > 0 ? "color:var(--red)" : "";
        return `<tr>
      ...
      historyTbody.innerHTML = `<tr><td colspan="4" style="text-align:center;color:var(--gray-400);padding:1rem;">No scores recorded yet.</td></tr>`;
    }
  }

  // My team info
  if (teamDiv) {
    const myTeam = teams.find(t => t.players.some(p => p.name.toLowerCase() === userName.toLowerCase()));
    if (myTeam) {
      const partner = myTeam.players.find(p => p.name.toLowerCase() !== userName.toLowerCase());
      const me = myTeam.players.find(p => p.name.toLowerCase() === userName.toLowerCase());
      const combined = myTeam.players[0].handicap + myTeam.players[1].handicap;
      teamDiv.innerHTML = `
        <div style="padding:1rem;">
          <div class="team-card" style="max-width:320px;">
            <div class="team-number">Team ${myTeam.id}</div>
            <div class="team-player">
              <span class="player-avatar-sm">${initials(me.name)}</span>
              <span class="team-player-name">${me.name} (You)</span>
              <span class="team-player-hcp">HCP ${me.handicap}</span>
            </div>
            <div class="team-player">
              <span class="player-avatar-sm">${initials(partner.name)}</span>
              <span class="team-player-name">${partner.name}</span>
              <span class="team-player-hcp">HCP ${partner.handicap}</span>
            </div>
            <div class="team-combined">
              <span>Combined HCP</span>
              <strong>${combined}</strong>
      ...
      teamDiv.innerHTML = `<p style="padding:1rem;color:var(--gray-400);">You are not currently assigned to a team.</p>`;
    }
  }
}
...
function applyRoleVisibility(role) {
  ...
  document.querySelectorAll("[data-role-require]").forEach(el => {
    const allowed = el.dataset.roleRequire.split(" ");
  ...
  // For player role, show "My Stats" instead of hidden items
  const myStatsNav = document.querySelector('.nav-item[data-section="my-stats"]');
  if (myStatsNav) {
    myStatsNav.style.display = role === "player" ? "" : "none";
  }
}

// ---- Navigation (updated with role support) ----
...
function renderTeamWeekSelect() {
  const sel = document.getElementById("teamWeekSelect");
  if (!sel) return;
  sel.innerHTML = weeklySchedule.map(w =>
    `<option value="${w.week}">Week ${w.week} — ${w.date} — ${w.course}</option>`
  ).join("");
  sel.addEventListener("change", renderMatchups);
}

function renderMatchups() {
  const weekNum = parseInt(document.getElementById("teamWeekSelect").value, 10);
  const week = weeklySchedule.find(w => w.week === weekNum);
  if (!week) return;

  const container = document.getElementById("matchupsPreview");

  function buildMatchupCards(pairings, sideLabel) {
    return pairings.map(([idA, idB]) => {
      const tA = teams.find(t => t.id === idA);
      const tB = teams.find(t => t.id === idB);
      if (!tA || !tB) return "";
      return `<div class="matchup-card">
        <div class="matchup-header">
          <span>Team ${idA} vs Team ${idB}</span>
          <span class="matchup-side-badge">${sideLabel}</span>
        </div>
        <div class="matchup-body">
          <div class="matchup-team">
            <span><strong>Team ${idA}:</strong> ${tA.players.map(p => p.name).join(" & ")}</span>
          </div>
          <div class="matchup-vs">VS</div>
          <div class="matchup-team">
            <span><strong>Team ${idB}:</strong> ${tB.players.map(p => p.name).join(" & ")}</span>
          </div>
        </div>
      </div>`;
    }).join("");
  ...
  container.innerHTML = `
    <h3 style="font-size:.92rem;font-weight:600;color:var(--green-900);padding:0 .5rem .5rem;">Front 9 Matchups</h3>
    <div class="matchup-grid">${buildMatchupCards(week.front, "Front 9")}</div>
    <h3 style="font-size:.92rem;font-weight:600;color:var(--green-900);padding:.5rem .5rem .5rem;">Back 9 Matchups</h3>
    <div class="matchup-grid">${buildMatchupCards(week.back, "Back 9")}</div>
  `;
}
...
function init() {
  ...
  renderPayments();
  renderTeamsGrid();
  renderTeamWeekSelect();
  renderMatchups();
  renderUserManagement();
  renderAdminTeamsGrid();
  renderHandicapTable();
  document.getElementById("scoreDate").value = new Date().toISOString().split("T")[0];

  // Listen for role-ready event from auth
  window.addEventListener("gltp-role-ready", function(e) {
    var user = e.detail || {};
  ...
  // If role already set (e.g. from cached session), apply immediately
  if (window.GLTP_USER && window.GLTP_USER.role) {
    applyRoleVisibility(window.GLTP_USER.role);
  }
}
